...
// Track Hash Changes dynamically
window.addEventListener('hashchange', () => {
    const hash = window.location.hash || '#home';
    
    if (hash === '#login') switchAuthView('login-view');
    else if (hash === '#register') switchAuthView('register-view');
    else if (hash === '#verify-otp') switchAuthView('verify-otp-view');
    else if (hash === '#create-password') switchAuthView('create-password-view');
    else if (hash === '#choose-experience') switchAuthView('choose-experience-view');
    else if (hash === '#business') switchAuthView('business-view');
    else if (hash === '#chat') switchAuthView('chat-view');
    else if (hash === '#' || hash === '#home') switchAuthView('landing-view');
});

// Trigger router on initial page load
...
if (registerForm) {
  ...
  const fields = ['full_name', 'email', 'mobile_local', 'password', 'confirm_password'];
...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (data.success) {
        window.location.href = 'choose-experience.html';
      } else {
